...
def test_energy_utilized_SSW():
    ...
    maxkW = 100
    maxkWh = 2.5 * maxkW
    instance, results = run_main(data, data, maxkWh, maxkW)
    print('Battery used when available')
...
def test_energy_SOC_tracks():
    ...
    print(purchased_power_isclose(instance, target))
    print(objective_is_right(target, data, instance))
    print(np.isclose(final, instance.stateOfCharge[0, 1].value))
...
def test_solar_utilized_over_battery():
    ...
    final = initial
    instance, results = run_main(data, data, maxkWh, maxkW)
    print('Solar used before battery')
    print(SSW_draw_isclosematch(instance, gph))
    print(np.isclose(instance.curtailedPower[0, 0].value, 0, rtol=1e-2), instance.curtailedPower[0, 0].value)
    print(np.isclose(instance.chargePower[0, 0].value, 0))
    print(np.isclose(instance.dischargePower[0, 0].value, 0), instance.dischargePower[0, 0].value)
    print(instance.beta_SSW[2, 0, 0].value, instance.beta_SSW[3, 0, 0].value)
    print(SSW_power_isclose_match(instance, powers[2]))
...
def test_excess_solar_curtailed():
    ...
    final = initial
    instance, results = run_main(data, data, maxkWh, maxkW)
    print('Curtail Excess Solar test')
    print(SSW_draw_isclosematch(instance, gph))
    target = powers[2] - maxkW
    print(SSW_power_isclose_match(instance, powers[2]))
    print(purchased_power_isclose(instance, 0))
...
def test_two_zone_solar_and_SSW():
    break_file, empty_file = define_empty_file()
    powers, flow_rates = read_breakpoints(break_file, 'SSW')
    data = pd.read_pickle(empty_file)[0:23]
    gph = flow_rates[2]
    data.SSW_demand.iloc[0] = gph
    solar_power = powers[2] * 2
    data.solar[0] = solar_power
    data.price = 1
    maxkW = 100
    maxkWh = 2.5 * maxkW
    instance, results = run_main(data, data, maxkWh, maxkW)
    print('Two zone solar test')
    print(instance.solar[0, 0] == solar_power, instance.solar[1, 0] == solar_power)

def two_zone_base_demand():
    break_file, empty_file = define_empty_file()
    data = pd.read_pickle(empty_file)[0:23]
    base_power = 100
    data.kW[0] = base_power
    data.price = 1
    instance, results = run_main(data, data)
    print(test_two_zone_base_demand(base_power, instance))

def two_zone_SSW_demand():
    break_file, empty_file = define_empty_file()
    powers, flow_rates = read_breakpoints(break_file, 'SSW')
    data = pd.read_pickle(empty_file)[0:23]
    gph = flow_rates[2]
    data.SSW_demand.iloc[0] = gph
    instance, results = run_main(data, data)
    print(test_two_zone_SSW_draw(gph, instance))


def test_two_zone_SSW_draw(gph, instance):
    print('test_two_zone_SSW_draw')
    return instance.SSW_draw[0, 0].value == gph, instance.SSW_draw[1, 0].value == gph


def test_two_zone_base_demand(base_power, instance):
    print('test_two_zone_base_demand')
    return instance.base_electrical[0, 0] == base_power, instance.base_electrical[1, 0] == base_power
...
def final_SOC_isclose(final, instance):
    value = instance.stateOfCharge[0, 1].value
    print(value)
...
def purchased_power_isclose(instance, target):
    value = instance.purchasedPower[0, 0].value
    return np.isclose(value, target)


def SSW_power_isclose_match(instance, target):
    value = instance.SSW_power[0, 0].value
    print('SSW_power_isclose_match', value, target)
    return np.isclose(value, target)

def SSW_draw_isclosematch(instance, target):
    value = instance.SSW_draw[0, 0].value
    print('SSW_draw_isclosematch')
    return np.isclose(value, target)

def DSW_power_isclose_match(instance, target):
    value = instance.DSW_power[0, 0].value
    print('DSW_power_isclose_match')
    return np.isclose(value, target)

def DSW_draw_isclosematch(target, instance):
    value = instance.DSW_draw[0, 0].value
    print('DSW_draw_isclosematch')
    return np.isclose(value, target)

def SSW_draw_matches(target, instance):
    print('SSW_draw_matches')
    return instance.SSW_draw[0, 0].value == target
# endregion
...
if __name__ == "__main__":
    ...
    test_two_zone_solar_and_SSW()
    two_zone_base_demand()
    two_zone_SSW_demand()
