...
BATTERY_EFF_INCREMENTS = [0.76,0.78,0.81,0.84,0.86]
# # solarIncrement = range(1, 6)
for i in BATTERY_EFF_INCREMENTS:
    ...
    model.Powertocurtail = Var(model.zone,model.month,model.time,domain=NonNegativeReals, bounds=(0,100000))
    ...
    def powercurtail(model,zone,month,time):
        return model.curtailedPower[zone,month,time] == (model.solar[zone,month,time] * -1 )+ model.wind_pow[zone,month,time] - model.Powertocurtail[zone,month,time]

    def limiting_power_curtail(model,zone,month,time):
        return model.curtailedPower[zone,month,time] <= (model.solar[zone,month,time] * -1 ) + model.wind_pow[zone,month,time]
    ...
    model.constraint_powercurtail = Constraint(model.zone,model.month,model.time,rule=powercurtail)
    ...
    def generate_write_folder(solar_increment, transferMax, Battery_cost_kw, Battery_cost_kwh):
        ...
        variable_eff_folder = '/sns_battery_efficiency_wind'

        write_folder = os.getcwd() + variable_eff_folder + Bat_efficiency
        return write_folder
