...
E2020B2030 = [(0.74,9),(0.84,10)]

E2020B2020 = [(0.86,10)]
for c in E2020B2030:
    battery_efficiency = [c[0]]
    INCREMENTS = [c[1]]
    elect_price_kw = {
        'old_2020': [13, 25]}
    ...
    ################ SOLOARIncrement #################
    for (i, j) in zip(elect_price_kw.values(), elect_price_kwh.values()):
        for (x, y) in zip(battery_kw, battery_kwh):
            for e in battery_efficiency:
                try:
                    for k in INCREMENTS:
                        try:
                            ...
                            wind_speed = [np.load('winddata.npy'), np.load('ff_wind_data.npy')]
                            ...
                            def winddata(model, zone, month, time):
                                return ((((wind_speed[zone][
                                               month - 1, time] / 2.23693629) ** 3) * 0.5 * 3.14 * blade_length * blade_length * air_density * wind_tur_eff) / 1000) * wind_increment
                            ...
                            model.SSW_pumpPower = Var(model.zone, model.month, model.time, domain=NonNegativeReals,
                                                      bounds=(0, Pmaxff))
                            model.DSW_pumpPower = Var(model.zone, model.month, model.time, domain=NonNegativeReals,
                                                      bounds=(0, Pmaxff))
                            model.peak_power = Var(model.zone, model.month, domain=NonNegativeReals, bounds=(0, Pmaxff))
                            model.SSW_pumped = Var(model.zone, model.month, model.time, domain=NonNegativeReals,
                                                   bounds=(0, maxFlowff))
                            model.DSW_pumped = Var(model.zone, model.month, model.time, domain=NonNegativeReals,
                                                   bounds=(0, maxFlowff))
                            model.transferSSW = Var(model.zone, model.month, model.time, domain=Reals,
                                                    bounds=(-transferMax, transferMax))
                            model.transferDSW = Var(model.zone, model.month, model.time, domain=Reals,
                                                    bounds=(-transferMax, transferMax))
                            model.chargePower = Var(model.zone, model.month, model.time, domain=NonNegativeReals,
                                                    bounds=(0, ESmax))
                            model.dischargePower = Var(model.zone, model.month, model.time, domain=NonNegativeReals,
                                                       bounds=(0, ESmax))
                            model.purchasedPower = Var(model.zone, model.month, model.time, domain=NonNegativeReals,
                                                       bounds=(0, Pmaxff))
                            model.stateOfCharge = Var(model.zone, model.month, model.time, domain=NonNegativeReals,
                                                      bounds=(0, SOCmax))
                            model.curtailedPower = Var(model.zone, model.month, model.time, domain=NonNegativeReals,
                                                       bounds=(0, 100000))
                            model.installed_capacity_kw = Var(model.zone, domain=NonNegativeReals, bounds=(ESmin, ESmax))
                            model.installed_capacity_kwh = Var(model.zone, domain=NonNegativeReals,
                                                               bounds=(ESmin, ESmax * 6))
                            ...
                            def SSW_gpm_to_pump_power(model, zone, month, time):
                                return model.SSW_pumpPower[zone, month, time] == A2_SSW[zone] * model.SSW_pumped[
                                    zone, month, time] * \
                            ...
                            def DSW_gpm_to_pump_power(model, zone, month, time):
                                return model.DSW_pumpPower[zone, month, time] == A2_DSW[zone] * model.DSW_pumped[
                                    zone, month, time] * \
                            ...
                            def stateOfCharge(model, zone, month, time):
                                ...
                                return model.stateOfCharge[zone, month, time] == SOCpre + (
                                            ...
                                            - model.dischargePower[
                                                zone, month, time] / discharge_eff) * dt
                            ...
                            def control_discharge_power(model, zone, month, time):
                                return model.dischargePower[zone, month, time] <= (
                                            1 - model.gamma[zone, month, time]) * ESmax


                            ##### BATTERY TEMPERATURE ####

                            def batterytemperature_gen(model, zone, month, time):
                                return model.bat_temp_gen[zone, month, time] == (1 - charge_eff) * model.chargePower[
                                    zone, month, time] \
                            ...
                            def batterytemperature(model, zone, month, time):
                                if time == 0:
                                    ...
                                    return model.battery_temperature[zone, month, time] == model.battery_temperature[
                                        zone, month, time - 1] + \
                                           (model.bat_temp_gen[zone, month, time] - ((convec_coeff * bat_area * (
                                                   model.battery_temperature[zone, month, time - 1] - model.envTemp[
                                               zone, month, time])))) \
                            ...
                            model.constraint_power_balance = Constraint(model.zone, model.month, model.time,
                                                                        rule=power_balance)
                            model.constraint_stateOfCharge = Constraint(model.zone, model.month, model.time,
                                                                        rule=stateOfCharge)
                            ...
                            model.constraint_SSW_gpm_to_pump_power = Constraint(model.zone, model.month, model.time,
                                                                                rule=SSW_gpm_to_pump_power)
                            model.constraint_DSW_gpm_to_pump_power = Constraint(model.zone, model.month, model.time,
                                                                                rule=DSW_gpm_to_pump_power)
                            model.constraint_stateOfCharge_and_discharge = Constraint(model.zone, model.month, model.time,
                                                                                      rule=stateOfCharge_and_discharge)
                            model.constraint_limits_stateOfCharge = Constraint(model.zone, model.month, model.time,
                                                                               rule=limits_stateOfCharge)
                            model.constraint_controlling_chargepower_shortmonthend = Constraint(model.zone, model.month,
                                                                                                model.time,
                                                                                                rule=controlling_chargepower_shortmonthend)
                            model.constraint_controlling_dischargepower_shortmonthend = Constraint(model.zone, model.month,
                                                                                                   model.time,
                                                                                                   rule=controlling_dischargepower_shortmonthend)
                            model.constraint_control_charge_power = Constraint(model.zone, model.month, model.time,
                                                                               rule=control_charge_power)
                            model.constraint_control_discharge_power = Constraint(model.zone, model.month, model.time,
                                                                                  rule=control_discharge_power)
                            model.constraint_bat_temp_gen = Constraint(model.zone, model.month, model.time,
                                                                       rule=batterytemperature_gen)
                            model.constraint_battery_temperature = Constraint(model.zone, model.month, model.time,
                                                                              rule=batterytemperature)
                            model.constraint_limiting_power_curtail = Constraint(model.zone, model.month, model.time,
                                                                                 rule=limiting_power_curtail)
                            model.constraint_to_turn_on_SSW_pump = Constraint(model.zone, model.month, model.time,
                                                                              rule=to_turn_on_SSW_pump)
                            model.constraint_to_turn_on_DSW_pump = Constraint(model.zone, model.month, model.time,
                                                                              rule=to_turn_on_DSW_pump)
                            model.constraint_peak_power = Constraint(model.zone, model.month, model.time, rule=peak_power)
                            model.constraint_maximum_cap_of_a_SSWpump = Constraint(model.zone, model.month, model.time,
                                                                                   rule=maximum_cap_of_a_SSWpump)
                            model.constraint_maximum_cap_of_a_DSWpump = Constraint(model.zone, model.month, model.time,
                                                                                   rule=maximum_cap_of_a_DSWpump)
                            model.constraint_transfer_SSWdirection = Constraint(model.month, model.time,
                                                                                rule=transfer_SSWdirection)
                            model.constraint_transfer_DSWdirection = Constraint(model.month, model.time,
                                                                                rule=transfer_DSWdirection)
                            model.constraint_transfer_DSWoneway = Constraint(model.month, model.time,
                                                                             rule=transfer_DSWoneWay)
                            model.constraint_transfer_SSWoneway = Constraint(model.month, model.time,
                                                                             rule=transfer_SSWoneWay)
                            ...
                            opt.options["IntFeasTol"] = 1e-09
                            opt.options["FeasibilityTol"] = 1e-07
                            # opt.options["OptimalityTol"] = 1e-09
                            ...
                            def generate_write_folder(solar_increment, transferMax, Battery_cost_kw, Battery_cost_kwh):
                                ...
                                variable_eff_folder = '/Sensitivity_study_for_ESS_DSM_SOLAR'
                            ...
                            Value_forgurobi = GRB.ERROR_NUMERIC
                            print('ERROR_CODE = ', Value_forgurobi)
                            if Value_forgurobi >> 10000:
                                continue
                except:
                    print("end of batteryefficiency")
    ...
    #
    #                     def winddata(model, zone, month, time):
    #                         return ((((wind_speed[zone][month - 1, time] / 2.23693629) ** 3) * 0.5 * 3.14 * blade_length * blade_length * air_density * wind_tur_eff) / 1000) * wind_increment
    #
    #
    #                     model.wind_pow = Param(model.zone, model.month, model.time, initialize=winddata)
    #
    ...
    #
    #                     def limiting_power_curtail(model, zone, month, time):
    #                         return model.curtailedPower[zone, month, time] <= solar_increment * -1 * model.solar[zone, month, time] + \
    #                                model.wind_pow[zone, month, time]
    #
    #
    #                     def maximum_cap_of_a_SSWpump(model, zone, month, time):
    ...
    #
    #                     opt.options["IntFeasTol"] = 1e-09
    #                     opt.options["FeasibilityTol"] = 1e-06
    #                     opt.options["OptimalityTol"] = 1e-09
    #                     opt.options['NodefileStart'] = 0.90
    #                     opt.options["NodeMethod"] = 1
    #                     opt.options["MIPGapAbs"] = 0.07
    ...
    #
    #                         # max_Transfer_limit_water_folder = '/ {}_max_transfer_of_water'.format(transferMax)
    #                         variable_eff_folder = '/Sensitivity_study_for_ESS_DSM_WIND'
    #
    #                         write_folder = os.getcwd() + variable_eff_folder + kWh_price_fol + Battery_cost_folder + Bat_efficiency + wind_incr
    #                         return write_folder
    #
    #
    ...
    #                         os.makedirs(write_folder)
    #
    #                     excel_filename = variable_eff_folder + '_' + 's_' + str(wind_increment) + 'xincr' + '_' + str(
    #                         kWh_price[0]) + '_' + str(Battery_cost_kw) + '_' + str(charge_eff)
    #
    #                     filename = '{}.xlsx'.format(excel_filename)
    #                     writer = pd.ExcelWriter(write_folder + filename)
    ...
    #                         continue
    #         except:
    #             print("end of batteryefficiency",j)